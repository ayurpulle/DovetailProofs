...
An exact predictive agent has enough information to return the
...
\begin{quote}
Which observation induced predictive structure must every exact predictive agent contain (or realise as a quotient)?
\end{quote}

Here predictive structure means the canonical predictive states together with their observation-labelled updates.
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item behavioural recovery of a controlled transition law from sufficiently
        ...
        stochastic and partially observable extensions due to Santiago
...
The contributions begin at the
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item framing $M_3$ and $N_5$ witnesses in that interval as inherited
        obstructions
  \item proving the reset monoid result for the stated full support
...
Make sure the probabilities of all possible continuations of a history add up to 1 (consistency). If the process is also stationary,
then its 'cylinder probabilities' (p(u) is the probability that the first $|u|$ observations are exactly the word u) additionally satisfy
\[
  p(u)=\sum_{o\in\Obs}p(ou).
\]
The process being stationary can be imposed, but none
of the quotient arguments uses it.

For a history $u$ with $p(u)>0$, define the conditional future law
\[
  q_u(v)=\frac{p(uv)}{p(u)}
  \qquad(v\in\Obs^*).
\]
...
If $p(u)=0$, consistency and non negativity imply $p(uv)=0$ $\forall$
$v$, and then conditional prediction after $u$ is undefined. Represent every such
impossible continuations by the dagger (not to be confused with Hermiticity)
$\dagger\notin X_p$.
...
\[
  X_p=\{q_u:p(u)>0\}.
\]
Dfn $\widehat X_p=X_p\cup\{\dagger\}$.
\end{definition}
...
\begin{itemize}
  ...
  \item $d\in Q$ is the dead state, the agent's version of
        $\dagger$, used for impossible continuations;
  \item each $\delta_o:Q\to Q$ is the state update on observing $o$;
  \item $F:Q^+\to X_p$ reads out the complete conditional future law
        represented by a live/possible state.
\end{itemize}
...
In words, impossible histories are absorbed into the dead state, changing no predictions and keeping update maps well defined. 
...
\begin{proof}
For a reachable state $q=\delta_u(q_0)$, set $L(q)=q_u$, and send the
...
\]
If $p(uo)=0$, both sides are $\dagger$. Both sides equal $\dagger$, so intertwining still holds for impossible observations.  The equality
also holds at the absorbing dead state.
\end{proof}

At the level of reachable possible state partitions, (Refinement Lemma of computational mechanics?)
 
The next step lifts that to transition monoids.

Let $S_A=\langle\delta_o:o\in\Obs\rangle\subseteq\mathcal T_Q$.

\begin{theorem}
\label{thm:monoid-quotient}
Under the previous hyopthesis, the assignment
\[
  \Phi(\delta_w)=\tau_w
\]
...
That development uses \texttt{FreeMonoid.lift}, where the convention is
opposite to the temporal convention above.  It constructs the
opposite transition monoid; the congruence conclusions transfer because
$\Con(S)=\Con(S^{\mathrm{op}})$. (Is this necesarily true?, go back to code)
...
\]
is a well-defined bijection.  Proportional rows become equal after
normalising their $\eps$ coordinate, which recovers $q_u$, and
...
sides. Then identify $X_p$ with this ray set.  The vector space $V$,
is the minimal linear realisation space.
...
Use a column convention.  For $w=o_1\cdots o_n$, put
\[
...
\]
In a general basis $\lambda$ is represented by a basis dependent covector.
...
argument as in Theorem~\ref{thm:monoid-quotient},
\[
...
This is the linear version of the predictor quotient at the level of
...
\]
Agreement on a spanning set of rays does not force
...
multiples.  Section~\ref{sec:reset} gives a process realisable strict inclusion.
...
\[
  ...
  \end{pmatrix}.
\]
The matrix is full and completely positive, and its three rows are distinct.  After any nonempty
...
\[
  ...
  \qquad c_i c_j=c_i,
\]
with $1$ an identity.  Every congruence that doesnt collapse the whole monoidhas $\{1\}$ as a lone
...
initial distribution is stricly positive and whose transition matrix is stricly positive and has pairwise distinct rows.  Then
\[
  S_p\cong R_n^1.
\]
Consequently,
...
subsystems.  A direct product decomposition requires suitable complementary factor
congruences and a congruence permutability condition.
...
This does not prove that every algebraic congruence is physically meaningful.  
It proves the conditional statement: once admissibility is defined as a stable lumping of transition operators, the admissible hierarchy is
$\Con(S)$.  Coarse-graining predictive states is a different construction.
...
states and give every trajectory zero reward.  Every policy is optimal,
including a controller with one live state (and the dead state) which
chooses one fixed action after every history.  A dead state
preserving map from this controller cannot surject onto two or more live
predictive states, and the same constant policy is optimal for a family
...
The counterexample is not restricted to a constant reward.  Histories
with different future laws may be control equivalent for a particular task, and
an optimal controller may merge them.  Richens prove a stronger
policy-level negative result for the class
$\Psi_{\mathrm{myopic}}$ of depth=1 goals requiring attainment immediately
after the first action, even a policy optimal for all such goals identifies no
nontrivial bound on the environment's transition probabilities, the
...
Santi removes two restrictions from this behavioural result.  The queried
...
There is also an important scope restriction.  In Santi's partially
observable theorem, goals are still formulated and evaluated using the latent
environment states, even though the policy observes only outputs.  He
...
\]
Under these hypotheses this is the state quotient required for monoid descent,
...
\subsection{an internal quotient}
\label{subsec:controller-formalism}

\paragraph{Status.}
This subsection reconnects the behavioural results above to the paper's monoid
theorem.  The algebraic descent step is complete; what remains unfinished is a
full proof that Richens--Santi recovery, in a suitable exact limit, yields a
\emph{state-local} recovery functional for a recurrent controller.  Until that
bridge is finished, Theorem~\ref{thm:controller-bridge} and
Corollary~\ref{cor:controller-con} should be read as conditional on the
state-local hypothesis rather than as unconditional consequences of behavioural
recovery alone.

Behavioural recovery in the style of Richens or Santi is policy-level.  To obtain
an internal predictive quotient it must factor through recurrent controller
state.  Once a surjective update-equivariant state map exists, the remainder of
the implication is the same descent used for exact predictors:
\[
  \begin{aligned}
    \text{state-local predictive recovery}
    &\Longrightarrow Q\rightarrow\widehat X_{\mathcal I}\\
    &\Longrightarrow S_C\rightarrow S_{\mathcal I}\\
    &\Longrightarrow
    \Con(S_{\mathcal I})
...
Here:
\begin{itemize}
  \item $Q$ is the set of controller states;
  \item $q_0\in Q$ is the initial state;
  \item $d\in Q$ is the absorbing dead state, the controller counterpart of
        $\dagger$;
  \item each $\delta_{a,o}:Q\to Q$ is the state update on the action--output pair
        $(a,o)$;
  \item $\pi:Q\times\mathcal G\to A$ is the deterministic goal-conditioned policy.
\end{itemize}
...
\[
  \Phi_C:S_C\rightarrow S_{\mathcal I}.
\]
...
The hypotheses above are exact.  Richens finite-depth
theorem guarantees only approximate behavioural recovery, so it does not by
itself produce an exact congruence interval.  One extension is to combine
Santis metric and predictive state separation
theorem with monoid descent to study persistence of finite $M_3/N_5$ witnesses
under controlled approximation \cite{CifuentesApprox2026}.

\subsection{Plan to finish this section}
\label{subsec:controller-plan}

Without relying on Nayebi, the remaining work is:
\begin{enumerate}[label=(\arabic*)]
  \item state precisely the exact-recovery limit of Richens--Santi behavioural
        recovery (e.g.\ vanishing relative failure and unbounded goal depth, or
        an exact identification special case);
  \item prove, or isolate as an explicit additional hypothesis, that this
        recovered information factors through a recurrent controller state,
        yielding a state-local recovery functional $\mathcal R$;
  \item verify that the resulting intertwiner meets the reachability, dead-state,
        and counterfactual-update conventions used above;
  \item only then promote Theorem~\ref{thm:controller-bridge} and
        Corollary~\ref{cor:controller-con} from conditional to unconditional
        consequences for that controller class;
  \item record the approximate case as future work via residual-metric robustness,
        without claiming an exact congruence interval at finite depth.
\end{enumerate}
The Nayebi internal-memory route remains available as an alternative upstream
input, but is left commented for now.
...
$\varepsilon$-transducer, Hankel, and behavioural-recovery results used to
reach that layer are prior work.  Failure of distributivity can be a
process-forced property of every exact predictor's transition monoid, but
without an additional criterion separating reset effects from stronger
algebraic constraints it is not a certificate of feature entanglement or
